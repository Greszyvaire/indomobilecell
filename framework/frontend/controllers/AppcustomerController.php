...
//            $data[$i]['city'] = (empty($city)) ? [] : $city->attributes;
    ...
    public function actionTest() {
        ...
        foreach ($models as $key => $val) {
            $created = ($val['created'] != '') ?  strtotime($val['created']) : strtotime('2015-11-17 19:58:41');
            $modified = ($val['modified'] != '') ?  strtotime($val['modified']) : strtotime('2015-11-17 19:58:41');
            $query[$i] = "UPDATE landa_indomobilecell.user SET created = '".$created."', modified ='".$modified."' WHERE user.id = ".$val['id'].";";
        $i++;
